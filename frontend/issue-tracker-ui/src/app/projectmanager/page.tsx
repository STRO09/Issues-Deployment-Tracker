...
import { FolderOpen, Bug, Rocket, ExternalLink, Plus, CheckCircle, AlertCircle, Clock } from "lucide-react"
...
export function ProjectManagerDashboard({ user }: ProjectManagerDashboardProps) {
  ...
  const getProjectProgress = (projectId: string) => {
    ...
    const closedIssues = projectIssues.filter((issue) => issue.status === "CLOSED").length
  ...
  const stats = {
    ...
    recentDeployments: deployments.filter(
      (d) => new Date(d.deployedAt) > new Date(Date.now() - 7 * 24 * 60 * 60 * 1000),
    ).length,
  }
  ...
  return (
    ...
    <div className="p-6 space-y-6">
      <div>
        <h1 className="text-3xl font-bold text-balance">Project Manager Dashboard</h1>
        <p className="text-muted-foreground mt-2">Overview of projects, issues, and deployments</p>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">Projects under management</p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{stats.recentDeployments}</div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between">
            <div>
              ...
              <CardDescription>Progress and status of your projects</CardDescription>
            </div>
            <Button size="sm">
                ...
                <div key={project.id} className="border rounded-lg p-4 space-y-3">
                  ...
                  <div className="flex items-center justify-between text-sm">
                    <div className="flex items-center gap-4">
                      <span className="text-muted-foreground">{projectIssues.length} issues</span>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Latest issues across your projects</CardDescription>
          </CardHeader>
          <CardContent>
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          <div>
            ...
            <CardDescription>Latest deployments across your projects</CardDescription>
          </div>
          <Button size="sm">
            ...
            <TableBody>
              {deployments.map((deployment) => (
                <TableRow key={deployment.id}>
                  <TableCell>
                    <div>
                      <div className="font-medium">{getProjectName(deployment.projectId)}</div>
                      <div className="text-sm text-muted-foreground">{deployment.description}</div>
                    </div>
                  </TableCell>
                  <TableCell>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Overview of team productivity and metrics</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="grid gap-4 md:grid-cols-3">
            <div className="text-center space-y-2">
              <div className="text-2xl font-bold text-green-500">85%</div>
              <div className="text-sm text-muted-foreground">Issues Resolved On Time</div>
            </div>
            <div className="text-center space-y-2">
              <div className="text-2xl font-bold text-blue-500">12</div>
              <div className="text-sm text-muted-foreground">Average Issues per Sprint</div>
            </div>
            <div className="text-center space-y-2">
              <div className="text-2xl font-bold text-purple-500">2.3 days</div>
